import React, { useState, useEffect } from "react";
  ...
  const LineChartForm = ({defaultConfig, defaultTile, onSubmit, onCancel,isEditing}) => {
  const [mode, setMode] = useState("manual");
  const [label, setLabel] = useState(defaultTile || "");
...
const handleFetchkeys = () => {
  ...
  if (!Array.isArray(xValues) || !Array.isArray(yValues)) {
    //alert("Chosen keys must point to array values.");
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg">
        <h2 className="text-xl font-semibold mb-4">Line Chart Configuration</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
         ...
         {mode === "device" && (
  <>
    {/* API URL Input and Validate IP Button */}
    <div>
...
{fetchdataStatus === "error" && (
  <p className="text-red-600 text-sm mt-2">Status: Failed to fetch data.</p>
    ...
    {/* Show key inputs after validation succeeds */}
    {showKeyInputs && (
      <>
        <div className="mt-2">
  ...
  <div>
    {fetchStatus === "success" ? (
      <>
        <p className="text-green-600 text-sm">Status: keys fetched successfully.</p>
      ...
      <p className="text-red-600 text-sm">Status: Failed to fetch keys. Chosen keys must point to array values.</p>
    )}
  </div>
)}
